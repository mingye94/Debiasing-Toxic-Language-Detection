...
def get_examples(fpath, env='dialect'):
    """
    ...
    with open(fpath, "r") as f:
        ...
        for line in reader:
            ...
            if env=='dialect':
                ...
                if len(idtyRe.findall(line[0]))>0:
                    e = 0
                elif len(oiRe.findall(line[0]))>0:
                    e = 1
                elif len(oniRe.findall(line[0]))>0:
                    e = 2
                else:
                    e = 3
    ...
    return np.array(ts), np.array(ys, dtype=np.float32), es
...
def get_toxic_datasets(data_dir, tr_pollution, max_seq_length=300, word_thres=2, env='mention'):
    """
    Generate toxic datasets.
    """
    ### training set
    print("Training set: ")
    t_tr, y_tr, e_tr = get_examples(os.path.join(data_dir, "train.tsv"), env=env)

    ### dev set
    print("Dev set: ")
    t_dev_, y_dev_, e_dev = get_examples(os.path.join(data_dir, "dev.tsv"), env=env)

    ### test set
    print("Test set: ")
    t_te_, y_te_, e_te = get_examples(os.path.join(data_dir, "tst.tsv"), env=env)
    ...
    texts = [str(x) for x in pt_tr] + [str(x) for x in pt_dev] + [str(x) for x in pt_te]
