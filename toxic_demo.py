...
from toxic import get_toxic_datasets
...
parser.add_argument('--log_path',
                    type=str,
                    default=None,
                    help='Path of log file [default: None]')
...
# load dataset
######################
args.vocab, D_tr, D_dev, D_te = get_toxic_datasets(args.data_dir,
                                                  args.tr_pollution,
                                                  env=args.env_type)
...
for epoch in range(args.num_epochs):
    ...
    # dev
    inv_acc, _, bias = test_imdb(D_dev, inv_rnn, args.log_path+'.dev.%d.txt'%epoch, args.vocab.idx2word)
    dev_results.append([inv_acc, bias])

    # test
    inv_acc, _, bias = test_imdb(D_te, inv_rnn, args.log_path+'.tst.%d.txt'%epoch, args.vocab.idx2word)
