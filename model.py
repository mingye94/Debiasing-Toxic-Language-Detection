...
class RnnEncoder(tf.keras.layers.Layer):
    ...
    def __init__(self, hdim):
        ...
        # self.rnn = tf.keras.layers.CuDNNGRU(units=hdim,
...
class InvRNN(tf.keras.Model):
    ...
    def call(self, inputs, masks, envs):
        ...
        #env_inv_enc_outputs = self.env_inv_encoder(env_inv_embeddings)

        # mask before max pooling
        # (1 - mask) * (-1e9) --> <pad> become very neg
        env_inv_enc_outputs_ = env_inv_enc_outputs * masks_ + (1. - masks_) * (-1e9)
        ...
        #env_enable_enc_inputs = tf.concat([env_enable_embeddings, envs_],
                #axis=-1)
        ...
        # mask before max pooling
        # (1 - mask) * (-1e9) --> <pad> become very neg
        env_enable_enc_outputs_ = env_enable_enc_outputs * masks_ + (1. - masks_) * (-1e9)
